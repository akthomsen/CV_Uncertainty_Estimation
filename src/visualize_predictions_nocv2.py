...
def main(
    ...
    print(f"Cutting down to 100 images for visualization")
    images_info = images_info[:100]

    # Loop over all images and visualize errors
    for image_info in tqdm(images_info, desc="Visualizing predictions", unit="image"):
        image_id = image_info['id']
        # print(image_info['file_name'])
        image_path = os.path.join(meta_catalog.image_root, image_info['file_name'])
        ...
        plt.imshow(plotted_detections.get_image())
        # plt.title('Detected Instances')
        plt.axis('off')
        output_dir = os.path.join(inference_output_dir, "visualizations")
        os.makedirs(output_dir, exist_ok=True)
        output_path = os.path.join(output_dir, f"{image_info['file_name'].split('.')[0]}_detections.pdf")
...
# EX: python src/visualize_predictions_nocv2.py --test-dataset bdd_val --inference-config Inference/bayes_od_mc_dropout.yaml --config-file BDD-Detection/retinanet/retinanet_R_50_FPN_1x_reg_cls_var_dropout.yaml --eval-only --random-seed 1 --dataset-dir BDD_DATASET_ROOT